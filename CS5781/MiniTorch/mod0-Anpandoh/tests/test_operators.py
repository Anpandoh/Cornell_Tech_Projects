...
def test_relu(a: float) -> None:
    """Check that the relu function is correct"""
    if a > 0:
        assert relu(a) == a
    if a < 0:
        assert relu(a) == 0.0
...
def test_relu_back(a: float, b: float) -> None:
    """Check that the relu back function is correct"""
    if a > 0:
...
def test_id(a: float) -> None:
    """Check that the identity function returns same value"""
    assert id(a) == a
...
def test_max(a: float) -> None:
    """Check that the max function returns the maximum value"""
...
def test_eq(a: float) -> None:
    """Check that the eq function returns 1.0 when equal"""
    assert eq(a, a) == 1.0
...
def test_transitive(a: float, b: float, c: float) -> None:
    """Checks the transitive property of less-than (a < b and b < c implies a < c)"""
    if lt(a, b) and lt(b, c):
        assert lt(a, c) is True


@pytest.mark.task0_2
@given(small_floats, small_floats)
def test_symmetric(a: float, b: float) -> None:
    """Checks that :func:`minitorch.operators.mul` is symmetric, i.e.
...
def test_distribute(a: float, b: float, c: float) -> None:
    """Checks that operators distribute, i.e.
...
@pytest.mark.task0_2
@given(small_floats, small_floats)
def test_associative(a: float, b: float) -> None:
    """Checks the associativity of the operators"""
