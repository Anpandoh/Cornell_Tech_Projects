...
void test_matmul_3x1_by_1x3(void)
{
    ...
    // Run function under test
    float **C = matmul_blocking(A, B, 3, 1, 1, 3);
...
void test_matmul_1024x1024(void)
{
    ...
    float **C = matmul_sparse(A, B, 1024, 1024, 1024, 1024);
...
void test_matmul_blocking_1024x1024(void)
{
    // Setup 1024x1024 matrix A (all elements are 2)
    float **A = (float **)malloc(1024 * sizeof(float *));
    for (int i = 0; i < 1024; i++)
    ...
    // Run the function under test
    float **C = matmul_blocking(A, B, 1024, 1024, 1024, 1024);
...
void test_sparse_matmul_1024x1024(void) {
    float sparsity = 1; // 1% non-zero elements
    // Setup 1024x1024 sparse matrix A (only a fraction of elements are non-zero)
    float **A = (float **)malloc(1024 * sizeof(float *));
    for (int i = 0; i < 1024; i++) {
        A[i] = (float *)malloc(1024 * sizeof(float));
        for (int j = 0; j < 1024; j++) {
            A[i][j] = ((float)rand() / RAND_MAX < sparsity) ? 2.0f : 0.0f;
        }
    }

    // Setup 1024x1024 sparse matrix B (only a fraction of elements are non-zero)
    float **B = (float **)malloc(1024 * sizeof(float *));
    for (int i = 0; i < 1024; i++) {
        B[i] = (float *)malloc(1024 * sizeof(float));
        for (int j = 0; j < 1024; j++) {
            B[i][j] = ((float)rand() / RAND_MAX < sparsity) ? 3.0f : 0.0f;
        }
    }
...
void test_sparse_matmul_blocking_1024x1024(void) {
    float sparsity = 1; // 1% non-zero elements
    // Setup 1024x1024 sparse matrix A (only a fraction of elements are non-zero)
    float **A = (float **)malloc(1024 * sizeof(float *));
    for (int i = 0; i < 1024; i++) {
...
void test_CSR_sparse_matmul_1024x1024(void) {
    float sparsity = 1; // 1% non-zero elements
    // Setup 1024x1024 sparse matrix A (only a fraction of elements are non-zero)
    float **A = (float **)malloc(1024 * sizeof(float *));
    for (int i = 0; i < 1024; i++) {
...
void test_matmul_2048x2048(void)
{
    // Setup 1024x1024 matrix A (all elements are 2)
    float **A = (float **)malloc(2048 * sizeof(float *));
    for (int i = 0; i < 2048; i++)
    {
        A[i] = (float *)malloc(2048 * sizeof(float));
    ...
    // Setup 1024x1024 matrix B (all elements are 3)
    float **B = (float **)malloc(2048 * sizeof(float *));
    for (int i = 0; i < 2048; i++)
    {
        B[i] = (float *)malloc(2048 * sizeof(float));
        for (int j = 0; j < 2048; j++)
        {
            B[i][j] = 3.0f;
        }
    ...
    // Expected result is a 1024x1024 matrix with every element as 6144 (2 * 3 * 1024)
    float **expected = (float **)malloc(2048 * sizeof(float *));
    for (int i = 0; i < 2048; i++)
    {
        expected[i] = (float *)malloc(2048 * sizeof(float));
        for (int j = 0; j < 2048; j++)
        {
            expected[i][j] = 12288.0f;
        }
    }

    // Run the function under test
    float **C = matmul(A, B, 2048, 2048, 2048, 2048);

    // Check expectations
    assert_float_array_equal_matmul(expected, C, 2048, 2048);
...
void test_matmul_blocking_2048x2048(void)
{
    // Setup 2048 matrix A (all elements are 2)
    float **A = (float **)malloc(2048 * sizeof(float *));
    for (int i = 0; i < 2048; i++)
    {
        A[i] = (float *)malloc(2048 * sizeof(float));
    ...
    // Setup 2048 matrix B (all elements are 3)
    float **B = (float **)malloc(2048 * sizeof(float *));
    for (int i = 0; i < 2048; i++)
    {
        B[i] = (float *)malloc(2048 * sizeof(float));
    ...
    // Expected result is a 2048 matrix with every element as 6144 (2 * 3 * 2048)
    float **expected = (float **)malloc(2048 * sizeof(float *));
    for (int i = 0; i < 2048; i++)
    {
        expected[i] = (float *)malloc(2048 * sizeof(float));
    ...
    // Run the function under test
    float **C = matmul_sparse(A, B, 2048, 2048, 2048, 2048);
...
{
    // Setup 1024x1024 matrix A (all elements are 2)
    float **A = (float **)malloc(512 * sizeof(float *));
    for (int i = 0; i < 512; i++)
    {
        A[i] = (float *)malloc(512 * sizeof(float));
        for (int j = 0; j < 512; j++)
        {
            A[i][j] = 2.0f;
        }
    ...
    // Setup 1024x1024 matrix B (all elements are 3)
    float **B = (float **)malloc(512 * sizeof(float *));
    for (int i = 0; i < 512; i++)
    {
        B[i] = (float *)malloc(512 * sizeof(float));
        for (int j = 0; j < 512; j++)
        {
            B[i][j] = 3.0f;
        }
    ...
    // Expected result is a 1024x1024 matrix with every element as 6144 (2 * 3 * 1024)
    float **expected = (float **)malloc(512 * sizeof(float *));
    for (int i = 0; i < 512; i++)
    {
        expected[i] = (float *)malloc(512 * sizeof(float));
        for (int j = 0; j < 512; j++)
        {
            expected[i][j] = 3072.0f;
        }
    }

    // Run the function under test
    float **C = matmul(A, B, 512, 512, 512, 512);

    // Check expectations
    assert_float_array_equal_matmul(expected, C, 512, 512);
...
void test_matmul_blocking_512x512(void)
{
    // Setup 512 matrix A (all elements are 2)
    float **A = (float **)malloc(512 * sizeof(float *));
    for (int i = 0; i < 512; i++)
    {
        A[i] = (float *)malloc(512 * sizeof(float));
    ...
    // Setup 512 matrix B (all elements are 3)
    float **B = (float **)malloc(512 * sizeof(float *));
    for (int i = 0; i < 512; i++)
    {
        B[i] = (float *)malloc(512 * sizeof(float));
    ...
    // Expected result is a 512 matrix with every element as 6144 (2 * 3 * 512)
    float **expected = (float **)malloc(512 * sizeof(float *));
    for (int i = 0; i < 512; i++)
    {
        expected[i] = (float *)malloc(512 * sizeof(float));
    ...
    // Run the function under test
    float **C = matmul_blocking(A, B, 512, 512, 512, 512);
