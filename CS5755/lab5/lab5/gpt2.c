...
float *linear(float *fcInput, float **weights, float *biases, int fcInputSize, int fcOutputSize) {
    ...
    for (int i = 0; i < fcOutputSize; i++) {
        ...
        int j = 0;
        __m256 sum = _mm256_setzero_ps();  //Set vec to 0
        // iterate by 8
        for (; j <= fcInputSize - 8; j += 8) {
            //load
            __m256 inputVec = _mm256_loadu_ps(&fcInput[j]);
            __m256 weightVec = _mm256_loadu_ps(&weights[i][j]);
            //fuse mul add
        ...
        // remainder
        for (; j < fcInputSize; j++) {
            dotProduct += fcInput[j] * weights[i][j];
        }
        output[i] += dotProduct;
    }
...
float **block(float **x, int seqLength, int embeddingSize, BlockWeights weights) {
    ...
    for (int i = 0; i < seqLength; i++) {
        ...
        float *first_output = linear(normalized_x_added[i], first_block_MLP.weights, first_block_MLP.biases, first_block_MLP.fcInputSize, first_block_MLP.fcOutputSize);
