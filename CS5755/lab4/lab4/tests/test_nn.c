...
{
    int numChannels = 3;
    int inputSize = 0;  // Set inputSize to 0 for an empty input
    float image_data[3][5][5] = {0}; // Empty data for each channel

    float ***image = init_image(image_data, inputSize, numChannels);
    float *output = flatten(image, inputSize, numChannels);
    const int outputSize = numChannels * inputSize * 5; // Should be 0 since inputSize is 0

    // Check that the output is NULL or an empty array
